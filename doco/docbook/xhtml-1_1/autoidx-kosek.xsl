...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:d="http://docbook.org/ns/docbook"
                xmlns:i="urn:cz-kosek:functions:index" xmlns:l="http://docbook.sourceforge.net/xmlns/l10n/1.0"
                ...
                extension-element-prefixes="func exslt" exclude-result-prefixes="func exslt i l k d" version="1.0">
    ...
    <xsl:template name="generate-kosek-index">
      ...
      <xsl:text>index extension functions be imported:
        ...
        <xsl:variable name="terms"
                      select="//d:indexterm[count(.|key('group-code', i:group-index(normalize-space(concat(d:primary/@sortas, &quot; &quot;, d:primary))))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1]) = 1 and not(@class = 'endofrange')]"/>
        ...
        <div class="index">
            <xsl:apply-templates select="$terms" mode="index-div-kosek">
                ...
                <xsl:sort select="i:group-index(normalize-space(concat(d:primary/@sortas, &quot; &quot;, d:primary)))"
                          data-type="number"/>
            </xsl:apply-templates>
        </div>
    </xsl:template>
    ...
    <xsl:template match="d:indexterm" mode="index-div-kosek">
        ...
        <xsl:variable name="key"
                      select="i:group-index(normalize-space(concat(d:primary/@sortas, &quot; &quot;, d:primary)))"/>
        ...
        <xsl:if test="key('group-code', $key)[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][count(.|key('primary', normalize-space(concat(d:primary/@sortas, &quot; &quot;, d:primary)))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1]) = 1]">
            <div class="indexdiv">
                ...
                <dl>
                    <xsl:apply-templates
                            select="key('group-code', $key)[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][count(.|key('primary', normalize-space(concat(d:primary/@sortas, &quot; &quot;, d:primary)))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1])=1]"
                            mode="index-primary">
