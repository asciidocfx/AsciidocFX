...
<xsl:stylesheet
    ...
    <xsl:param
            name="epub.vocabulary.profile.content">http://www.idpf.org/epub/30/profile/content/
    </xsl:param>
    <xsl:param
            name="epub.vocabulary.profile.package">http://www.idpf.org/epub/30/profile/package/
    </xsl:param>
    ...
    <xsl:template name="metadata.modified">
        ...
        <xsl:choose>
            <xsl:when test="string-length($utc.datetime) != 0">
                ...
                <xsl:comment>The preceding date value is actually local time (not UTC) in UTC format because there is no
                    function in XSLT 1.0 to generate a correct UTC time
                </xsl:comment>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="d:book|
        ...
        <xsl:element name="navPoint" namespace="{$ncx.namespace}">
            ...
            <xsl:if test="$depth != 0">
                ...
                <xsl:apply-templates
                        select="d:book[parent::d:set]|d:part|d:reference|d:preface|d:chapter|d:bibliography|d:appendix|d:article|d:topic|d:glossary|d:section|d:sect1|d:sect2|d:sect3|d:sect4|d:sect5|d:refentry|d:colophon|d:bibliodiv[d:title]|d:setindex|d:index"
                        mode="ncx"/>
            </xsl:if>
        </xsl:element>
