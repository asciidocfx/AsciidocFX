...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:d="http://docbook.org/ns/docbook"
                ...
                xmlns:exsl="http://exslt.org/common" xmlns:exslt="http://exslt.org/common" exslt:dummy="dummy"
                ng:dummy="dummy" db:dummy="dummy" extension-element-prefixes="exslt" version="1.0"
                exclude-result-prefixes="exsl db ng exslt d">
    ...
    <xsl:template
            match="d:book|d:part|d:reference|d:preface|d:chapter|d:bibliography|d:appendix|d:article|d:glossary|d:section|d:sect1|d:sect2|d:sect3|d:sect4|d:sect5|d:refentry|d:colophon|d:bibliodiv|d:index"
            mode="etoc">
        <xsl:variable name="title">
            <xsl:if test="$eclipse.autolabel=1">
                <xsl:variable name="label.markup">
        ...
        <topic label="{normalize-space($title)}" href="{$href}">
            <xsl:apply-templates
                    select="d:part|d:reference|d:preface|d:chapter|d:bibliography|d:appendix|d:article|d:glossary|d:section|d:sect1|d:sect2|d:sect3|d:sect4|d:sect5|d:refentry|d:colophon|d:bibliodiv|d:index"
                    mode="etoc"/>
        </topic>
    ...
    <xsl:template name="plugin.xml">
        <xsl:call-template name="write.chunk">
            ...
            <xsl:with-param name="quiet" select="$chunk.quietly"/>
            <xsl:with-param name="content">
                <plugin name="{$eclipse.plugin.name}" id="{$eclipse.plugin.id}" version="1.0"
            ...
            <xsl:choose>
                <xsl:when test="$rootid != ''">
                    <xsl:apply-templates select="key('id',$rootid)//d:indexterm" mode="idx">
                        <xsl:sort
                                select="normalize-space(concat(d:primary/@sortas, d:primary[not(@sortas) or @sortas = '']))"/>
                        <xsl:sort
                                select="normalize-space(concat(d:secondary/@sortas, d:secondary[not(@sortas) or @sortas = '']))"/>
                        <xsl:sort
                                select="normalize-space(concat(d:tertiary/@sortas, d:tertiary[not(@sortas) or @sortas = '']))"/>
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="d:indexterm|d:primary|d:secondary|d:tertiary" mode="idx">
        ...
        <xsl:variable name="text">
            ...
            <xsl:if test="following-sibling::*[1][self::d:see]">
                ...
                <xsl:text> </xsl:text>
                <xsl:value-of select="following-sibling::*[1][self::d:see]"/>)
            </xsl:if>
        </xsl:variable>
