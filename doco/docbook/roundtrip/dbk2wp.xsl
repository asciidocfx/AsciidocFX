<xsl:stylesheet version="1.0"
    ...
    <xsl:template
            match='*[contains(local-name(), "d:info")]/*[not(self::d:title|self::d:subtitle|self::d:titleabbrev|self::doc:title|self::doc:subtitle|self::doc:titleabbrev)]'
            priority='0'
            mode='doc:body'/>
    ...
    <xsl:template match='d:address[not(parent::d:author|parent::d:editor|parent::d:othercredit)] |
                  ...
                  mode='doc:body'>
        <xsl:apply-templates
                select='node()[not(self::d:affiliation|self::d:authorblurb|self::doc:affiliation|self::doc:authorblurb)]'/>
    </xsl:template>
    ...
    <xsl:template match='d:caption|doc:caption'
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:for-each
                        select='text()|*[not(self::d:para|self::doc:para)]|*[self::d:para|self::doc:para][position() != 1]'>
                    <xsl:call-template name='doc:nomatch'/>
                </xsl:for-each>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match='d:entry|doc:entry' mode='doc:body'>
        ...
        <xsl:variable name='width.raw'>
            <xsl:choose>
                    ...
                    <xsl:variable name='combinedWidth'>
                        <xsl:call-template name='doc:sum'>
                            <xsl:with-param name='nodes'
                                            select='ancestor::*[self::d:table|self::doc:table|self::d:informaltable|self::doc:informaltable][1]/*[self::d:tgroup|self::doc:tgroup]/*[self::d:colspec|self::doc:colspec][not(position() &lt; $position) and position() &lt; $limit]'/>
                ...
                <xsl:otherwise>
                    <xsl:value-of
                            select='ancestor::*[self::d:table|self::doc:table|self::d:informaltable|self::doc:informaltable][1]/*[self::d:tgroup|self::doc:tgroup]/*[self::d:colspec|self::doc:colspec][position() = $position]/@colwidth'/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template
            match='*[self::d:para|self::d:simpara|self::doc:para|self::doc:simpara]/text()[string-length(normalize-space(.)) != 0]'
            mode='doc:body'>
        <xsl:call-template name='doc:handle-linebreaks'/>
    </xsl:template>

    <xsl:template
            match='text()[not(parent::d:para|parent::d:simpara|parent::d:literallayout|parent::d:programlisting | parent::doc:para|parent::doc:simpara|parent::doc:literallayout|parent::doc:programlisting)][string-length(normalize-space(.)) != 0]'
            mode='doc:body'>
        <xsl:call-template name='doc:handle-linebreaks'/>
    </xsl:template>
    ...
    <xsl:template match='d:blockquote|doc:blockquote'
        ...
        <xsl:apply-templates
                select='*[not(self::d:blockinfo|self::d:title|self::d:attribution|self::doc:info|self::doc:title|self::doc:attribution)]'
                mode='doc:body'>
