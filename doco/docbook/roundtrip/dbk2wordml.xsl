<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/" name='wordml.top'>
        ...
        <xsl:if test='not($wordml.template)'>
            <xsl:message terminate='yes'>Please specify the template document with the "wordml.template" parameter
            </xsl:message>
        </xsl:if>
        <xsl:if test='not($templatedoc)'>
            <xsl:message terminate='yes'>Unable to open template document "<xsl:value-of select='$wordml.template'/>"
            </xsl:message>
        </xsl:if>
        ...
        <xsl:variable name='authors'
                      select='$info/d:author|$info/d:authorinitials|$info/d:authorgroup/d:author|$info/d:authorgroup/d:editor'/>
        ...
        <w:wordDocument
                    ...
                    <xsl:variable name="content">
                        <xsl:choose>
                            <xsl:when test='$info/d:revhistory/d:revision[1]/*[self::d:author|self::d:authorinitials]'>
                                <xsl:apply-templates
                                        select='$info/d:revhistory/d:revision[1]/*[self::d:author|self::d:authorinitials]'
            ...
            <xsl:apply-templates
                    select='$templatedoc/w:wordDocument/o:CustomDocumentProperties|$templatedoc/w:wordDocument/w:fonts|$templatedoc/w:wordDocument/w:lists|$templatedoc/w:wordDocument/w:styles'
                    mode='doc:copy'/>
    ...
    <xsl:template name='doc:make-table'>
        ...
        <xsl:param name='content'>
            <xsl:apply-templates
                    select='*[not(self::d:caption|self::doc:caption|self::d:textobject|self::doc:textobject)]'
                    mode='doc:body'/>
        </xsl:param>
    ...
    <xsl:template name='attributes'>
        ...
        <xsl:if test='$node/@*'>
            ...
            <w:r>
                ...
                <aml:annotation aml:id='{count(preceding::*) + 1}' aml:author="DocBook"
                                aml:createdate='2004-12-23T00:01:00' w:type='Word.Comment' w:initials='DBK'>
                    <aml:content>
                        <w:p>
                            <w:pPr>
