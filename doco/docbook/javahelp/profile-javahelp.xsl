...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:d="http://docbook.org/ns/docbook"
                xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:ng="http://docbook.org/docbook-ng"
                ...
                extension-element-prefixes="exslt" version="1.0" exclude-result-prefixes="doc ng db exsl exslt d">
    ...
    <xslo:variable xmlns:xslo="http://www.w3.org/1999/XSL/Transform" name="profiled-content">
        <xslo:choose>
            <xslo:when test="namespace-uri(*[1]) != 'http://docbook.org/ns/docbook'">
                <xsl:message>Adding DocBook namespace to version 4 DocBook document</xsl:message>
                <xsl:variable name="addns">
                    <xsl:apply-templates mode="addNS" select="/"/>
                </xsl:variable>
                <xsl:apply-templates select="exsl:node-set($addns)" mode="profile"/>
            </xslo:when>
            <xslo:otherwise>
    ...
    <xsl:template match="/">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                    ...
                    <xsl:when test="$rootid != ''">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:apply-templates select="$profiled-nodes//*[@id=$rootid or @xml:id=$rootid]"
                                                     mode="process.root"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:when>
                ...
                <xsl:for-each
                        select="$profiled-nodes">    <!-- This is just a hook for building profiling stylesheets -->
    ...
    <xsl:template name="helpset">
        <xsl:call-template name="write.chunk.with.doctype">
            ...
            <xsl:with-param name="doctype-public"
                            select="'-//Sun Microsystems Inc.//DTD JavaHelp HelpSet Version 1.0//EN'"/>
    ...
    <xsl:template name="helptoc">
        <xsl:call-template name="write.chunk.with.doctype">
            ...
            <xsl:with-param name="indent" select="'yes'"/>
            <xsl:with-param name="doctype-public"
                            select="'-//Sun Microsystems Inc.//DTD JavaHelp TOC Version 1.0//EN'"/>
    ...
    <xsl:template match="d:book" mode="jhtoc">
        ...
        <tocitem target="{$id}">
            ...
            <xsl:apply-templates
                    select="d:part|d:reference|d:preface|d:chapter|d:appendix|d:article|d:colophon|d:glossary|d:bibliography"
                    mode="jhtoc"/>
        </tocitem>
    </xsl:template>
    ...
    <xsl:template name="helpmap">
        <xsl:call-template name="write.chunk.with.doctype">
            ...
            <xsl:with-param name="indent" select="'yes'"/>
            <xsl:with-param name="doctype-public"
                            select="'-//Sun Microsystems Inc.//DTD JavaHelp Map Version 1.0//EN'"/>
    ...
    <xsl:template name="helpmap.content">
        <map version="1.0">
            <xsl:choose>
                <xsl:when test="$rootid != ''">
                    <xsl:apply-templates
                            select="key('id',$rootid)//d:set                                      | key('id',$rootid)//d:book                                      | key('id',$rootid)//d:part                                      | key('id',$rootid)//d:reference                                      | key('id',$rootid)//d:preface                                      | key('id',$rootid)//d:chapter                                      | key('id',$rootid)//d:appendix                                      | key('id',$rootid)//d:article                                      | key('id',$rootid)//d:colophon                                      | key('id',$rootid)//d:refentry                                      | key('id',$rootid)//d:section                                      | key('id',$rootid)//d:sect1                                      | key('id',$rootid)//d:sect2                                      | key('id',$rootid)//d:sect3                                      | key('id',$rootid)//d:sect4                                      | key('id',$rootid)//d:sect5                                      | key('id',$rootid)//d:indexterm                                       | key('id',$rootid)//d:glossary                                      | key('id',$rootid)//d:bibliography          | key('id',$rootid)//*[@id]"
                            mode="map"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates
                            select="//d:set                                      | //d:book                                      | //d:part                                      | //d:reference                                      | //d:preface                                      | //d:chapter                                      | //d:appendix                                      | //d:article                                      | //d:colophon                                      | //d:refentry                                      | //d:section                                      | //d:sect1                                      | //d:sect2                                      | //d:sect3                                      | //d:sect4                                      | //d:sect5                                      | //d:indexterm                                      | //d:glossary                                      | //d:bibliography          | //*[@id]"
                            mode="map"/>
                </xsl:otherwise>
            </xsl:choose>
        </map>
    ...
    <xsl:template
            match="d:part|d:reference|d:preface|d:chapter|d:appendix|d:refentry|d:article|d:glossary|d:bibliography"
            mode="map">
        <xsl:variable name="id">
            <xsl:call-template name="object.id"/>
        </xsl:variable>
    ...
    <xsl:template name="helpidx">
        <xsl:call-template name="write.chunk.with.doctype">
            ...
            <xsl:with-param name="indent" select="'yes'"/>
            <xsl:with-param name="doctype-public"
                            select="'-//Sun Microsystems Inc.//DTD JavaHelp Index Version 1.0//EN'"/>
        ...
        <xsl:choose>
            <xsl:when test="d:see">
                <xsl:variable name="see">
                    <xsl:value-of select="normalize-space(d:see)"/>
                </xsl:variable>
