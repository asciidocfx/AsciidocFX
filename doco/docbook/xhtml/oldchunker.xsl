...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:d="http://docbook.org/ns/docbook"
                ...
                xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns="http://www.w3.org/1999/xhtml" version="1.1"
                exclude-result-prefixes="doc d" extension-element-prefixes="saxon redirect lxslt">
    ...
    <doc:param xmlns="" name="saxon.character.representation">
        <refpurpose xmlns="http://www.w3.org/1999/xhtml">Saxon character representation used in generated HTML pages
        </refpurpose>
        <refdescription xmlns="http://www.w3.org/1999/xhtml">
    ...
    <xsl:template name="write.chunk">
        ...
        <xsl:choose>
            <xsl:when test="contains($vendor, 'SAXON 6.2')">
                ...
                <xsl:document href="{$filename}" method="{$method}" encoding="{$encoding}" indent="{$indent}"
            ...
            <xsl:when test="contains($vendor, 'SAXON')">
                ...
                <saxon:output file="{$filename}" href="{$filename}" method="{$method}" encoding="{$encoding}"
                              indent="{$indent}" saxon:character-representation="{$saxon.character.representation}">
                    <xsl:copy-of select="$content"/>
                </saxon:output>
            </xsl:when>
            <xsl:when test="contains($vendor, 'Apache')">
    ...
    <xsl:template name="write.chunk.with.doctype">
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains($vendor, 'SAXON')">
                <!-- Saxon uses saxon:output -->
                <saxon:output file="{$filename}" href="{$filename}" method="{$method}" encoding="{$encoding}"
                              indent="{$indent}" doctype-public="{$doctype-public}" doctype-system="{$doctype-system}"
                              saxon:character-representation="{$saxon.character.representation}">
                    <xsl:copy-of select="$content"/>
                </saxon:output>
            </xsl:when>
