...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:d="http://docbook.org/ns/docbook"
                ...
                xmlns:ptbl="http://nwalsh.com/xslt/ext/xsltproc/python/Table" xmlns="http://www.w3.org/1999/xhtml"
                exclude-result-prefixes="doc stbl xtbl lxslt ptbl d" version="1.0">
    ...
    <xsl:template name="empty.table.cell">
        ...
        <xsl:variable name="rowsep">
            <xsl:choose>
                ...
                <xsl:when
                        test="not(ancestor-or-self::d:row[1]/following-sibling::d:row                           or ancestor-or-self::d:thead/following-sibling::d:tbody                           or ancestor-or-self::d:tbody/preceding-sibling::d:tfoot)">
    ...
    <xsl:template match="d:entry|d:entrytbl" name="entry">
        ...
        <xsl:variable name="cellgi">
            <xsl:choose>
                ...
                <xsl:when
                        test="ancestor::d:tbody and                        (ancestor::d:table[@rowheader = 'firstcol'] or                       ancestor::d:informaltable[@rowheader = 'firstcol']) and                       ancestor-or-self::d:entry[1][count(preceding-sibling::d:entry) = 0]">
        ...
        <xsl:variable name="rowsep">
            <xsl:choose>
                <!-- If this is the last row, rowsep never applies. -->
                <xsl:when
                        test="ancestor::d:entrytbl                       and not (ancestor-or-self::d:row[1]/following-sibling::d:row)                       and not (ancestor::d:thead)">
                    <xsl:value-of select="0"/>
                </xsl:when>
                <xsl:when
                        test="not(ancestor-or-self::d:row[1]/following-sibling::d:row                           or ancestor-or-self::d:thead/following-sibling::d:tbody                           or ancestor-or-self::d:tbody/preceding-sibling::d:tfoot)">
                ...
                <xsl:when
                        test="@morerows and not(@morerows &lt;                   count(ancestor-or-self::d:row[1]/following-sibling::d:row))">
                    <xsl:value-of select="0"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:call-template name="inherited.table.attribute">
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="following-sibling::d:entry|following-sibling::d:entrytbl">
                        <xsl:apply-templates
                                select="(following-sibling::d:entry                                        |following-sibling::d:entrytbl)[1]">
                    ...
                    <xsl:when test="$colspec.colnum=$countcol">
                        <col>
                            <xsl:choose>
                                ...
                                <xsl:when
                                        test="$colspec/@colwidth and                              not($colspec/parent::*/d:colspec/@colwidth[contains(.,'*')])">
                                    <xsl:attribute name="width">
                                        <xsl:choose>
                                            <xsl:when test="normalize-space($colspec/@colwidth) = '*'">
                                                <xsl:value-of select="'1*'"/>
