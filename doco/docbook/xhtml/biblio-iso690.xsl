...
<xsl:stylesheet exclude-result-prefixes="d"
    ...
    <xsl:template name="iso690.makecitation">
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test="./@role='contribution' or (./d:biblioset/@relation='part' and ./d:biblioset/@relation='book')">
                <xsl:call-template name="iso690.paper.mon"/>
            </xsl:when>

            <!-- ARTICLES, ETC., IN SERIALS -->
            <xsl:when
                    test="./@role='article' or (./d:biblioset/@relation='journal' and ./d:biblioset/@relation='article')">
                <xsl:call-template name="iso690.article"/>
            </xsl:when>

            <!-- PATENT DOCUMENTS -->
            <xsl:when
                    test="./@role='patent' or (./d:bibliomisc[@role='patenttype'] and ./d:bibliomisc[@role='patentnum'])">
    ...
    <xsl:template match="d:biblioset" mode="iso690.article.jour">
        ...
        <xsl:call-template name="iso690.edition">
            <xsl:with-param name="after"
                            select="./d:pubdate[not(@role='issuing')]|./d:volumenum|./d:issuenum|./d:pagenums"/>
        </xsl:call-template>
                            ...
                            <xsl:when test="($count = $person.count)">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:call-template name="iso690.endsep">
                                            <xsl:with-param name="text"
                                                            select="string($person.list[position()=$count]//d:firstname[1])"/>
                                            <xsl:with-param name="sep" select="$primary.sep"/>
                                        </xsl:call-template>
                                    </xsl:otherwise>
                                </xsl:choose>
    ...
    <xsl:template name="iso690.pub">
        ...
        <xsl:choose>
            <xsl:when
                    test="(./d:publisher/d:publishername|./d:publishername|./d:publisher/d:address/d:city)and($onlydate=0)and(./d:pubdate[not(@role='issuing')]|./d:copyright/d:year|./d:date[@role='upd']|./d:date[@role='upd'])">
            ...
            <xsl:when
                    test="(./d:publisher/d:publishername|./d:publishername)and(./d:publisher/d:address/d:city)and($onlydate=0)">
                <xsl:apply-templates mode="iso690.mode" select="./d:publisher/d:address/d:city">
                    <xsl:with-param name="sep" select="$placesep"/>
                </xsl:apply-templates>
                <xsl:apply-templates mode="iso690.mode" select="./d:publisher/d:publishername|./d:publishername">
    ...
    <xsl:template name="iso690.pat.ident">
        ...
        <xsl:choose>
            <xsl:when test="./d:biblioid[@class='other' and @otherclass='patentnum']">
                <xsl:apply-templates mode="iso690.mode"
                                     select="./d:biblioid[@class='other' and @otherclass='patentnum']"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <!-- Number -->
    <xsl:template match="d:biblioid[@class='other' and @otherclass='patentnum']|d:bibliomisc[@role='patentnum']"
                  mode="iso690.mode">
