...
<xsl:stylesheet exclude-result-prefixes="d"
    ...
    <xsl:template match="d:itemizedlist">
        ...
        <div>
            ...
            <xsl:apply-templates
                    select="*[not(self::d:listitem                   or self::d:title                   or self::d:titleabbrev)]                 |comment()[not(preceding-sibling::d:listitem)]                 |processing-instruction()[not(preceding-sibling::d:listitem)]"/>
            ...
            <ul>
                ...
                <xsl:apply-templates
                        select="d:listitem                     |comment()[preceding-sibling::d:listitem]                     |processing-instruction()[preceding-sibling::d:listitem]"/>
            </ul>
        </div>
    </xsl:template>
            ...
            <xsl:choose>
                <xsl:when test="@inheritnum='inherit' and ancestor::d:listitem[parent::d:orderedlist]">
                    <table border="{$table.border.off}">
                        ...
                        <tbody>
                            <xsl:apply-templates mode="orderedlist-table"
                                                 select="d:listitem                         |comment()[preceding-sibling::d:listitem]                         |processing-instruction()[preceding-sibling::d:listitem]"/>
                        </tbody>
                    </table>
                </xsl:when>
    ...
    <xsl:template match="d:orderedlist/d:listitem">
        <li>
            ...
            <xsl:if test="@override">
                <xsl:message>
                    <xsl:text>@override attribute cannot be set in strict XHTML output for listitem: </xsl:text><xsl:value-of
                        select="@override"/>
                </xsl:message>
            </xsl:if>
        ...
        <div>
            ...
            <xsl:choose>
                <xsl:when test="$presentation = 'table'">
                    <!-- Preserve order of PIs and comments -->
                    <xsl:apply-templates
                            select="*[not(self::d:varlistentry                     or self::d:title                     or self::d:titleabbrev)]                   |comment()[not(preceding-sibling::d:varlistentry)]                   |processing-instruction()[not(preceding-sibling::d:varlistentry)]"/>
                    <table border="{$table.border.off}">
                        <xsl:call-template name="generate.class.attribute">
                            <xsl:with-param name="class" select="$default.class"/>
                        </xsl:call-template>
                        ...
                        <tbody>
                            <xsl:apply-templates mode="varlist-table"
                                                 select="d:varlistentry                       |comment()[preceding-sibling::d:varlistentry]                       |processing-instruction()[preceding-sibling::d:varlistentry]"/>
                        </tbody>
                    </table>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="d:procedure">
        ...
        <xsl:variable name="preamble"
                      select="*[not(self::d:step                   or self::d:title                   or self::d:titleabbrev)]                 |comment()[not(preceding-sibling::d:step)]                 |processing-instruction()[not(preceding-sibling::d:step)]"/>
            ...
            <xsl:choose>
                <xsl:when test="count(d:step) = 1">
                    <ul>
                        <xsl:call-template name="generate.class.attribute"/>
                        <xsl:apply-templates
                                select="d:step                     |comment()[preceding-sibling::d:step]                     |processing-instruction()[preceding-sibling::d:step]"/>
                    </ul>
                </xsl:when>
                <xsl:otherwise>
                    <ol>
    ...
    <xsl:template match="d:substeps">
        ...
        <xsl:variable name="cssstyle">
            <xsl:choose>
                <xsl:when test="$numeration = '1'">decimal</xsl:when>
                <xsl:when test="$numeration = 'a'">lower-alpha</xsl:when>
                <xsl:when test="$numeration = 'i'">lower-roman</xsl:when>
                <xsl:when test="$numeration = 'A'">upper-alpha</xsl:when>
                <xsl:when test="$numeration = 'I'">upper-roman</xsl:when>
                <xsl:otherwise>
                    <xsl:message>Warning: unknown procedure.step.numeration value:
                        <xsl:value-of select="$numeration"/>
                    </xsl:message>
                </xsl:otherwise>
            </xsl:choose>
        ...
        <ol>
            <xsl:attribute name="style">
                <xsl:text>list-style-type: </xsl:text><xsl:value-of select="$cssstyle"/>
            </xsl:attribute>
            <xsl:apply-templates/>
        </ol>
    </xsl:template>
    ...
    <xsl:template match="d:calloutlist">
        <div>
            ...
            <!-- Preserve order of PIs and comments -->
            <xsl:apply-templates
                    select="*[not(self::d:callout or self::d:title or self::d:titleabbrev)]                    |comment()[not(preceding-sibling::d:callout)]                    |processing-instruction()[not(preceding-sibling::d:callout)]"/>
            ...
            <xsl:choose>
                <xsl:when test="$callout.list.table != 0">
                    <table border="{$table.border.off}">
                        ...
                        <xsl:apply-templates
                                select="d:callout                                 |comment()[preceding-sibling::d:callout]                                 |processing-instruction()[preceding-sibling::d:callout]"/>
                    </table>
                </xsl:when>
                <xsl:otherwise>
                    <dl>
