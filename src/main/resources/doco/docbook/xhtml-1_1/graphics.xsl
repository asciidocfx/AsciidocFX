...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:d="http://docbook.org/ns/docbook"
                ...
                exclude-result-prefixes="xlink stext xtext lxslt simg ximg d" extension-element-prefixes="stext xtext"
                version="1.0">
    ...
    <xsl:template name="process.image">
        ...
        <xsl:variable name="realintrinsicwidth">
            ...
            <xsl:choose>
                <xsl:when
                        test="$use.extensions != 0 and $graphicsize.extension != 0                       and not(@format='SVG')">
                    <xsl:choose>
        ...
        <xsl:variable name="viewport">
            <xsl:choose>
                <xsl:when test="$ignore.image.scaling != 0">0</xsl:when>
                <xsl:when
                        test="local-name(.) = 'inlinegraphic'                       or ancestor::d:inlinemediaobject                       or ancestor::d:inlineequation">
                    0
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="d:programlisting/d:inlinemediaobject                      |d:screen/d:inlinemediaobject"
                  priority="2">
    ...
    <xsl:template match="d:imageobjectco">
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test="$use.role.for.mediaobject != 0 and                     count(d:imageobject) &gt; 1 and                     d:imageobject[@role]">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="phrases"
                              select="ancestor::d:mediaobject/d:textobject[d:phrase]                             |ancestor::d:inlinemediaobject/d:textobject[d:phrase]                             |ancestor::d:mediaobjectco/d:textobject[d:phrase]"/>
    ...
    <xsl:template match="d:caption">
        <div>
            ...
            <xsl:if test="@align = 'right' or @align = 'left' or @align='center'">
                <xsl:attribute name="style">
                    <xsl:text>text-align: </xsl:text><xsl:value-of select="@align"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:apply-templates/>
        </div>
