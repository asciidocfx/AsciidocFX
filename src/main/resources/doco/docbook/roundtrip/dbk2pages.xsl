<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/" name='pages.top'>
        ...
        <xsl:if test='not($pages.template)'>
            <xsl:message terminate='yes'>Please specify the template document with the "pages.template" parameter
            </xsl:message>
        </xsl:if>
        <xsl:if test='not($templatedoc)'>
            <xsl:message terminate='yes'>Unable to open template document "<xsl:value-of select='$pages.template'/>"
            </xsl:message>
        </xsl:if>
        ...
        <sf:calc-engine-entities>
            ...
            <xsl:for-each select='$rnd:pages-tables'>
                ...
                <sf:tabular-model sfa:ID='SFTTableModel-{$num}' sf:name='Unnamed Table'
                                  sfa:id=''
                                  sf:num-header-rows='{count(doc:tgroup/doc:thead/doc:row)}'
                                  sf:num-footer-rows='{count(doc:tgroup/doc:tfoot/doc:row)}'
                        ...
                        <sf:datasource sfa:ID='SFTConcreteTableDataSource-{$num}'>
                            ...
                            <xsl:apply-templates select='doc:tgroup/doc:tfoot/doc:row'
                                ...
                                <xsl:with-param name='start-row'
                                                select='count(doc:tgroup/doc:thead/doc:row) + count(doc:tgroup/doc:tbody/doc:row)'/>
                            </xsl:apply-templates>
                        </sf:datasource>
    ...
    <xsl:template name='doc:make-body'>
        ...
        <sf:text-storage sf:kind='body' sfa:ID='SFWPStorage-7'>
            ...
            <sf:text-body>
                ...
                <sf:container-hint sf:page-index="0" sf:cindex="0" sf:sindex="0" sf:lindex="0"
                                   sf:frame-x="56.692913055419922" sf:frame-y="56.692913055419922"
                                   sf:frame-w="481.61416625976562" sf:frame-h="714" sf:anchor-loc="0"/>
    ...
    <xsl:template name='doc:lookup-paragraph-style'>
        ...
        <xsl:if test='not($paragraph-styles[@sf:name = $style.cooked])'>
            <xsl:message>unable to find paragraph style "<xsl:value-of select='$style.cooked'/>"
            </xsl:message>
        </xsl:if>
    ...
    <xsl:template name='doc:lookup-character-style'>
        ...
        <xsl:if test='not($character-styles[@sf:name = $style])'>
            <xsl:message>unable to find character style "<xsl:value-of select='$style'/>"
            </xsl:message>
        </xsl:if>
