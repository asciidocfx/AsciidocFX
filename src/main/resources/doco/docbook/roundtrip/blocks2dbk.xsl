...
<xsl:stylesheet exclude-result-prefixes="d"
                ...
                <xsl:choose>
                    <xsl:when test='translate(substring-after(@rnd:style, "d:list"), "0123456789", "") != "" or
                        ...
                        <xsl:call-template name='rnd:error'>
                            ...
                            <xsl:with-param name='message'>style "<xsl:value-of select='@rnd:style'/>" is not a valid
                                list style
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
                    ...
                    <xsl:when
                            test='preceding-sibling::dbk:para[1][starts-with(@rnd:style, "d:itemizedlist") or starts-with(@rnd:style, "d:orderedlist") or @rnd:style = "para-continue"]'/>
                    ...
                    <xsl:when test='$stop.node'>
                        <xsl:element name='{substring-before(@rnd:style, "1")}'
                                     namespace='http://docbook.org/ns/docbook'>
                            <xsl:apply-templates
                                    select='.|following-sibling::dbk:para[@rnd:style = current()/@rnd:style][following-sibling::*[generate-id() = generate-id($stop.node)]]'
                                    mode='rnd:listitem'/>
                        </xsl:element>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:element name='{substring-before(@rnd:style, "1")}'
                                     namespace='http://docbook.org/ns/docbook'>
                            <xsl:apply-templates
                                    select='.|following-sibling::dbk:para[@rnd:style = current()/@rnd:style]'
                                    mode='rnd:listitem'/>
                        </xsl:element>
                    </xsl:otherwise>
                </xsl:choose>
            ...
            <xsl:when test='@rnd:style = "d:programlisting" or
                ...
                <xsl:element name='{@rnd:style}'
                    ...
                    <xsl:choose>
                        <xsl:when test='$stop.node'>
                            <xsl:apply-templates
                                    select='following-sibling::dbk:para[following-sibling::*[generate-id() = generate-id($stop.node)]]'
            ...
            <xsl:when test='@rnd:style = "sidebar-title"'>
                ...
                <dbk:sidebar>
                    ...
                    <xsl:choose>
                        <xsl:when test='$stop.node'>
                            <xsl:apply-templates
                                    select='following-sibling::*[following-sibling::*[generate-id() = generate-id($stop.node)]]'
                                    mode='rnd:sidebar'/>
                        </xsl:when>
                        <xsl:otherwise>
          ...
          -->
            ...
            <xsl:when test='@rnd:style = "blockquote-attribution"'>
                <xsl:call-template name='rnd:error'>
                    ...
                    <xsl:with-param name='message'>blockquote attribution must follow a blockquote title
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
                      ...
                      @rnd:style = "blockquote-attribution")][1]'/>
                        ...
                        <dbk:blockquote>
                            ...
                            <xsl:choose>
                                <xsl:when test='$stop.node'>
                                    <xsl:apply-templates
                                            select='following-sibling::*[following-sibling::*[generate-id() = generate-id($stop.node)]][@rnd:style = "blockquote-attribution"]'
                                            mode='rnd:blockquote-attribution'/>
					       ...
					       following-sibling::*[following-sibling::*[generate-id() = generate-id($stop.node)]]'
                                ...
                                <xsl:otherwise>
                                    <xsl:apply-templates
                                            select='following-sibling::*[@rnd:style = "blockquote-attribution"]'
                                            mode='rnd:blockquote-attribution'/>
                                    <xsl:apply-templates select='self::*[@rnd:style = "d:blockquote"] |
					       following-sibling::*'
                                                         mode='rnd:blockquote'/>
                                ...
                                <dbk:imageobject>
                                    <dbk:imagedata>
                                        <xsl:attribute name='fileref'>
                                            <xsl:choose>
                                                <xsl:when
                                                        test='dbk:inlinemediaobject/dbk:imageobject/dbk:imagedata/@fileref != ""'>
                                                    <xsl:value-of
                                                            select='dbk:inlinemediaobject/dbk:imageobject/dbk:imagedata/@fileref'/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select='.'/>
                                                </xsl:otherwise>
            ...
            <xsl:otherwise>
                <xsl:call-template name='rnd:error'>
                    ...
                    <xsl:with-param name='message'>unknown paragraph style "<xsl:value-of select='@rnd:style'/>"
                        encountered
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
    ...
    <xsl:template name='rnd:figure-text-caption'>
        ...
        <xsl:choose>
            ...
            <xsl:when test='$textobjs[self::dbk:informaltable |
                      ...
                      @rnd:style = "mediaobject-imagedata"]]'/>
            <!-- caption belongs to something else -->
            <xsl:otherwise>
    ...
    <xsl:template match='dbk:emphasis'>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name='rnd:error'>
                    <xsl:with-param name='code'>unknown-style</xsl:with-param>
                    <xsl:with-param name='message'>unknown character span style "<xsl:value-of select='@rnd:style'/>"
                        encountered
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
			...
			$metadata'>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test='not($next.captioned)'>
                            <xsl:apply-templates
                                    select='ancestor::dbk:para/following-sibling::*[following-sibling::*[generate-id() = generate-id($caption)]][not(&metadata-content;)]'
                                    mode='rnd:figure'/>
    ...
    <xsl:template match='dbk:para' mode='rnd:listitem'>
        <dbk:listitem>
            ...
            <xsl:choose>
                ...
                <xsl:when
                        test='following-sibling::*[self::dbk:para and @rnd:style != "para-continue"][1][@rnd:style != concat("d:itemizedlist", $list-level + 1) and @rnd:style != concat("d:orderedlist", $list-level + 1)]'/>
                ...
                <xsl:when test='following-sibling::dbk:para[@rnd:style = concat("d:itemizedlist", $list-level + 1) or @rnd:style = concat("d:orderedlist", $list-level + 1)] and
                    ...
                    <xsl:element name='{concat(substring-before($nested/@rnd:style, "list"), "list")}'
                                 namespace='http://docbook.org/ns/docbook'>
                        <xsl:apply-templates
                                select='following-sibling::dbk:para[@rnd:style = concat("d:itemizedlist", $list-level + 1) or @rnd:style = concat("d:orderedlist", $list-level + 1)][following-sibling::*[generate-id() = generate-id($stop.node)]]'
                                mode='rnd:listitem'/>
                    </xsl:element>
                </xsl:when>
                <xsl:when
                        test='following-sibling::dbk:para[@rnd:style = concat("d:itemizedlist", $list-level + 1) or @rnd:style = concat("d:orderedlist", $list-level + 1)]'>

                    <xsl:element name='{concat(substring-before($nested/@rnd:style, "list"), "list")}'
                                 namespace='http://docbook.org/ns/docbook'>
                        <xsl:apply-templates
                                select='following-sibling::dbk:para[@rnd:style = concat("d:itemizedlist", $list-level + 1) or @rnd:style = concat("d:orderedlist", $list-level + 1)]'
                                mode='rnd:listitem'/>
                    </xsl:element>
                </xsl:when>
            </xsl:choose>
        ...
        <xsl:choose>
            ...
            <xsl:when test='@rnd:style = "biblioentry-title"'>
                <xsl:call-template name='rnd:error'>
                    ...
                    <xsl:with-param name='message'>style "<xsl:value-of select='@rnd:style'/>" must not be metadata for
                        parent "<xsl:value-of select='local-name(..)'/>"
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:when>
            ...
            <xsl:when test='@rnd:style = "abstract-title" or
                ...
                <xsl:choose>
                    <xsl:when test='$stop.node'>
                        <dbk:abstract>
                            <xsl:apply-templates
                                    select='.|following-sibling::dbk:para[@rnd:style = "d:abstract"][following-sibling::*[generate-id() = generate-id($stop.node)]]'
                                    mode='rnd:abstract'/>
                        </dbk:abstract>
            ...
            <xsl:when test='@rnd:style = "d:legalnotice"'>
                ...
                <xsl:choose>
                    <xsl:when test='$stop.node'>
                        <dbk:legalnotice>
                            <xsl:apply-templates
                                    select='.|following-sibling::dbk:para[@rnd:style = "d:legalnotice"][following-sibling::*[generate-id() = generate-id($stop.node)]]'
                        ...
                        <xsl:when test='dbk:emphasis[@rnd:style = "d:orgname"]'>
                            ...
                            <xsl:if test='*[not(@rnd:style = "d:orgname")]'>
                                <xsl:call-template name='rnd:error'>
                                    ...
                                    <xsl:with-param name='message'>character span "<xsl:value-of
                                            select='dbk:emphasis[@rnd:style != "d:orgname"][1]/@rnd:style'/>" not
                                        allowed in an author paragraph combined with orgname
                                    </xsl:with-param>
                                </xsl:call-template>
                            </xsl:if>
                        </xsl:when>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name='rnd:error'>
                            ...
                            <xsl:with-param name='message'>titleabbrev style "<xsl:value-of select='@rnd:style'/>"
                                mismatches parent "<xsl:value-of select='local-name(..)'/>"
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name='rnd:error'>
                            ...
                            <xsl:with-param name='message'>title style "<xsl:value-of select='@rnd:style'/>" mismatches
                                parent "<xsl:value-of select='local-name(..)'/>"
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:otherwise>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:call-template name='rnd:error'>
                            ...
                            <xsl:with-param name='message'>subtitle style "<xsl:value-of select='@rnd:style'/>"
                                mismatches parent "<xsl:value-of select='local-name(..)'/>"
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:otherwise>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name='rnd:error'>
                    ...
                    <xsl:with-param name='message'>character span "<xsl:value-of select='@rnd:style'/>" not allowed in
                        an author paragraph
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
    ...
    <xsl:template name='rnd:attributes'>
        ...
        <xsl:apply-templates select='$node/@*[namespace-uri() != "http://docbook.org/d:ns/d:docbook/d:roundtrip"]'
                             mode='rnd:copy'/>
    </xsl:template>
    ...
    <xsl:template name='rnd:error'>
        ...
        <xsl:message>ERROR "<xsl:value-of select='$code'/>":
            <xsl:value-of select='$message'/>
        </xsl:message>
        <rnd:error>
            <rnd:code>
    ...
    <xsl:template name='rnd:warning'>
        ...
        <xsl:message>WARNING "<xsl:value-of select='$code'/>":
            <xsl:value-of select='$message'/>
        </xsl:message>
        <rnd:warning>
            <rnd:code>
