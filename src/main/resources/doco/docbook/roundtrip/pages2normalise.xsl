<xsl:stylesheet version="1.0"
            ...
            <xsl:otherwise>
                <dbk:informaltable>
                  ...
                  @sf:row &lt; $num-rows - $model/@sf:num-footer-rows]'/>
                                <xsl:with-param name='num-rows'
                                                select='$num-rows - $model/@sf:num-header-rows - $model/@sf:num-footer-rows'/>
    ...
    <xsl:template name='rnd:make-table-rows'>
        ...
        <xsl:choose>
            <xsl:when test='not($nodes) and $num-rows != 0'>
                ...
                <xsl:comment>WARNING: insufficient table cells (num-rows <xsl:value-of select='$num-rows'/>, row
                    <xsl:value-of select='$row'/>)
                </xsl:comment>
            </xsl:when>
            <xsl:when test='$nodes and $num-rows = 0'>
                <xsl:message>WARNING: excess table cells</xsl:message>
                <xsl:comment>WARNING: excess table cells (num-rows <xsl:value-of select='$num-rows'/>, row <xsl:value-of
                        select='$row'/>)
                </xsl:comment>
            </xsl:when>
    ...
    <xsl:template match='*'>
        <xsl:message>element "<xsl:value-of select='name()'/>" not handled
        </xsl:message>
    </xsl:template>
    ...
    <xsl:template name='rnd:find-style'>
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test='key("d:styles", $ident)/self::sf:characterstyle/sf:property-map/sf:superscript/sf:number/@sfa:number = "1"'>
                superscript
            </xsl:when>
            <xsl:when
                    test='key("d:styles", $ident)/self::sf:characterstyle/sf:property-map/sf:subscript/sf:number/@sfa:number = "1"'>
                subscript
            </xsl:when>
