...
. Firstly, install http://www.oracle.com/technetwork/java/javase/downloads/index.html[JDK 8]
. Download http://maven.apache.org/download.cgi[Apache Maven] and set `/bin` directory to environment variables
...
Tested with Ubuntu 14.04 (LTS), Mint 17.3, and Ubuntu 16.04 (LTS).

Creating a runnable build::
...
** wget --continue http://download-keycdn.ej-technologies.com/install4j/install4j_linux_6_0_4.deb
** sudo dpkg -i install4j_linux_6_0_4.deb
. Register your install4j license key and download JREs to /opt/install4j6/jres.
** /opt/install4j6/bin/install4j
. [__Workaround for location of install4j in pom.xml__]:
** sudo mkdir -p /home/travis/build/asciidocfx/AsciidocFX
** sudo ln -s /opt/install4j6/ /home/travis/build/asciidocfx/AsciidocFX/install4j6
. Create release builds. Run the following command from folder AsciidocFX:
** mvn package
. Builds are in folder AsciidocFX/target/media
